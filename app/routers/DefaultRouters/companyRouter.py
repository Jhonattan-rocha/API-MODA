...
async def create_company(company: companySchema.CompanyBase, db: AsyncSession = Depends(database.get_db), validation: str = Depends(verify_token)):
...
async def read_company(company_id: int, db: AsyncSession = Depends(database.get_db), validation: str = Depends(verify_token)):
...
async def delete_company(company_id: int, db: AsyncSession = Depends(database.get_db), validation: str = Depends(verify_token)):
