...
async def create_category(input_output_stock: inputOutputStockSchema.InputOutputStockBase, db: AsyncSession = Depends(database.get_db)):
...
async def read_categories(skip: int = 0, limit: int = 10, db: AsyncSession = Depends(database.get_db)):
...
async def read_category(input_output_stock_id: int, db: AsyncSession = Depends(database.get_db)):
...
async def delete_category(input_output_stock_id: int, db: AsyncSession = Depends(database.get_db)):
