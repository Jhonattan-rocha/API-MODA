...
async def create_employee(employee: employeeSchema.EmployeesBase, db: AsyncSession = Depends(database.get_db), validation: str = Depends(verify_token)):
...
async def read_employee(employee_id: int, db: AsyncSession = Depends(database.get_db), validation: str = Depends(verify_token)):
...
async def delete_employee(employee_id: int, db: AsyncSession = Depends(database.get_db), validation: str = Depends(verify_token)):
