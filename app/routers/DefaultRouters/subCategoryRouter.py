...
async def read_subcategory(subcategory_id: int, db: AsyncSession = Depends(database.get_db)):
...
async def delete_subcategory(subcategory_id: int, db: AsyncSession = Depends(database.get_db)):
