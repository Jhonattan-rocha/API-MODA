...
async def read_product(product_category_id: int, db: AsyncSession = Depends(database.get_db)):
...
async def delete_product(product_category_id: int, db: AsyncSession = Depends(database.get_db)):
