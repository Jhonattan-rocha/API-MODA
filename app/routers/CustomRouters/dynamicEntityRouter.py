...
async def create_dynamic_entity(dynamic_entity: dynamicEntities.DynamicEntityCreate, db: AsyncSession = Depends(database.get_db)):
...
async def read_dynamic_entities(skip: int = 0, limit: int = 10, db: AsyncSession = Depends(database.get_db)):
    return await dynamic_entities_controller.get_dynamic_entities(skip=skip, limit=limit, db=db)


@router.get("/dynamic_entities/{product_id}", response_model=dynamicEntities.DynamicEntity)
async def read_dynamic_entity(dynamic_entity_id: int, db: AsyncSession = Depends(database.get_db)):
...
async def delete_dynamic_entity(dynamic_entity_id: int, db: AsyncSession = Depends(database.get_db)):
