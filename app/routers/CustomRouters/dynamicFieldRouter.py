...
async def read_dynamic_fields(filters: str = None, skip: int = 0, limit: int = 10,
                                db: AsyncSession = Depends(database.get_db),
                                validation: str = Depends(verify_token)):
    return await dynamic_fields_controller.get_dynamic_fields(skip=skip, limit=limit, db=db, filters=filters, model="DynamicFields")


@router.get("/dynamic_fields/{dynamic_field_id}", response_model=dynamicFields.DynamicFields)
async def read_dynamic_field(dynamic_field_id: int, db: AsyncSession = Depends(database.get_db), validation: str = Depends(verify_token)):
    return await dynamic_fields_controller.get_dynamic_field(dynamic_field_id=dynamic_field_id, db=db)


@router.put("/dynamic_fields/{dynamic_field_id}", response_model=dynamicFields.DynamicFieldsCreate)
async def update_dynamic_field(dynamic_field_id: int, updated_dynamic_field: dynamicFields.DynamicFieldsCreate,
                               db: AsyncSession = Depends(database.get_db), validation: str = Depends(verify_token)):
...
async def delete_dynamic_field(dynamic_field_id: int, db: AsyncSession = Depends(database.get_db), validation: str = Depends(verify_token)):
